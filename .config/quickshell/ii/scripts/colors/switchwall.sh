            ...
            if [[ "$action" == "open_upscayl" ]]; then
                if command -v upscayl &>/dev/null; then
                    ...
                    if [[ "$action2" == "install_upscayl" ]]; then
                        kitty -- bash -c "yay -S upscayl-bin"
                        if command -v upscayl &>/dev/null; then
                            nohup upscayl > /dev/null 2>&1 &
                        fi
                    fi
...
set_wallpaper_path() {
    local path="$1"
    if [ -f "$SHELL_CONFIG_FILE" ]; then
        if temp_json=$(jq --arg path "$path" '.background.wallpaperPath = $path' "$SHELL_CONFIG_FILE"); then
            echo "$temp_json" > "$SHELL_CONFIG_FILE"
        fi
    fi
}
...
set_thumbnail_path() {
    local path="$1"
    if [ -f "$SHELL_CONFIG_FILE" ]; then
        if temp_json=$(jq --arg path "$path" '.background.thumbnailPath = $path' "$SHELL_CONFIG_FILE"); then
            echo "$temp_json" > "$SHELL_CONFIG_FILE"
        fi
    fi
}
...
switch() {
    ...
    cursorposx=$(hyprctl cursorpos -j | jq -r '.x // empty' 2>/dev/null)
    [[ -z "$cursorposx" ]] && cursorposx=960
    cursorposx=$(bc <<< "scale=0; ($cursorposx - $screenx) * $scale / 1")
    cursorposy=$(hyprctl cursorpos -j | jq -r '.y // empty' 2>/dev/null)
    [[ -z "$cursorposy" ]] && cursorposy=540
    ...
    if [[ "$color_flag" == "1" ]]; then
        ...
        if [[ "$imgpath" == *"[WE-"* ]]; then
            ...
            # Setup array to cleanly handle multi-monitor native execution into one single client process!
            local we_args=(--fps 60)
            while read -r monitor; do
                we_args+=(--screen-root "$monitor")
            done < <(hyprctl monitors -j | jq -r '.[] | .name')
            
            # Pass the direct directory path as the final positional argument
            we_args+=("$imgpath")
            ...
            # Restore Script for WE (using exact same fixed logic)
            cat > "$RESTORE_SCRIPT.tmp" << EOF
#!/bin/bash
...
EOF
            ...
            if [ ${#missing_deps[@]} -gt 0 ]; then
                ...
                if [[ "$action" == "install_arch" ]]; then
                    kitty -- bash -c "sudo pacman -S ${missing_deps[*]}"
            ...
            ffmpeg -y -i "$imgpath" -vframes 1 "$thumbnail" > "$STATE_DIR/user/generated/ffmpeg_thumb.log" 2>&1
    ...
    # Execute Matugen
    echo "Exporting colors with Matugen..."
    /home/zatch/.local/bin/matugen "${matugen_args[@]}"
...
main() {
    ...
    set_accent_color() {
        local color="$1"
        if temp_json=$(jq --arg color "$color" '.appearance.palette.accentColor = $color' "$SHELL_CONFIG_FILE"); then
            echo "$temp_json" > "$SHELL_CONFIG_FILE"
        fi
    }
