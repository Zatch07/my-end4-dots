  ...
  PanelWindow {
    ...
    ListView {
      ...
      delegate: Item {
        ...
        // Image container (gradient bg, live preview, icon overlay, parallelogram mask)
        Item {
          id: imageContainer
          anchors.fill: parent

          // Background gradient using matugen surface colors
          Rectangle {
            anchors.fill: parent
            gradient: Gradient {
              GradientStop {
                ...
                color: windowSwitcher.colors
                  ? Qt.rgba(windowSwitcher.colors.surfaceContainer.r,
                            windowSwitcher.colors.surfaceContainer.g,
                            windowSwitcher.colors.surfaceContainer.b, 1)
                  : "#1a1c2e"
              }
              GradientStop {
                position: 0.5
                color: windowSwitcher.colors
                  ? Qt.rgba(windowSwitcher.colors.surface.r * 0.9 + windowSwitcher.colors.primary.r * 0.1,
                            windowSwitcher.colors.surface.g * 0.9 + windowSwitcher.colors.primary.g * 0.1,
                            windowSwitcher.colors.surface.b * 0.9 + windowSwitcher.colors.primary.b * 0.1, 1)
                  : "#141622"
              }
              GradientStop {
                position: 1.0
                color: windowSwitcher.colors
                  ? Qt.rgba(windowSwitcher.colors.surface.r,
                            windowSwitcher.colors.surface.g,
                            windowSwitcher.colors.surface.b, 1)
                  : "#0e1018"
              }
            }
          }

          // Match this window's hyprctl address to a Wayland ToplevelManager entry
          // so ScreencopyView can capture its live framebuffer.
          property var toplevelSource: {
            var _trigger = ToplevelManager.toplevels; // force QML re-evaluation
            var rawId = typeof model.winId === 'string' ? model.winId : model.winId.toString();
            // Normalize: strip leading "0x" for comparison (end-4 pattern)
            var needle = rawId.replace(/^0x/i, "").toLowerCase();
            var tops = ToplevelManager.toplevels.values;
            for (var i = 0; i < tops.length; ++i) {
               var ht = tops[i].HyprlandToplevel;
               if (ht) {
                   var tlAddr = (ht.address || "").toLowerCase();
                   if (tlAddr === needle) {
                       return tops[i];
                   }
               }
          ...
          // Live window preview — scaled to fill the parallelogram height,
          // centered horizontally so the mask clips the sides naturally.
          Item {
            id: previewContainer
            anchors.fill: parent
            clip: false
            ...
            ScreencopyView {
              id: windowThumb
              // Scale to fill the full height, maintain aspect ratio
              property real winAspect: (model.winWidth > 0 && model.winHeight > 0)
                  ? (model.winWidth / model.winHeight) : (16/9)
              height: parent.height
              width: height * winAspect
              anchors.verticalCenter: parent.verticalCenter
              anchors.horizontalCenter: parent.horizontalCenter
              captureSource: imageContainer.toplevelSource
              live: true
              visible: imageContainer.toplevelSource !== null
            }
          }

          // Dim overlay for non-focused windows
          Rectangle {
            anchors.fill: parent
            color: Qt.rgba(0, 0, 0, delegateItem.isCurrent ? 0 : 0.35)
            Behavior on color { ColorAnimation { duration: 200 } }
          }

          // App icon from system icon theme (using Quickshell.iconPath API)
          Rectangle {
            id: iconCircle
            anchors.centerIn: parent
            ...
            radius: 28
            color: windowSwitcher.colors
              ? Qt.rgba(windowSwitcher.colors.surface.r, windowSwitcher.colors.surface.g, windowSwitcher.colors.surface.b, 0.7)
              : Qt.rgba(0, 0, 0, 0.5)
            border.width: 2
            border.color: delegateItem.isCurrent
              ? (windowSwitcher.colors ? windowSwitcher.colors.primary : "#4fc3f7")
              : Qt.rgba(1, 1, 1, 0.15)
            opacity: windowThumb.visible ? 0.9 : 1.0
            ...
            Image {
              ...
              width: 36
              height: 36
              // Quickshell.iconPath resolves the icon name from the system theme
              source: Quickshell.iconPath(model.appId, "image-missing")
              sourceSize: Qt.size(72, 72)
              smooth: true
            }
            // Fallback letter when no system icon found
            Text {
              anchors.centerIn: parent
